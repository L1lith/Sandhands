...
function sanitizeArray(sanitizeAny, input, format, options) {
  ...
  if (!options.hasOwnProperty('firstAsStandard') && !options.hasOwnProperty('standard') && format.length === 1) firstAsStandard = true // Default to true only if there's only one entry in the array.
  if (options.firstAsStandard === true && options.hasOwnProperty('standard')) throw new Error("Cannot both specify standard manually and set firstAsStandard to true.")
